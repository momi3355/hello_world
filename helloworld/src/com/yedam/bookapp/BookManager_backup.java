...
public class BookManager_backup {
	private enum Menu {
		NONE   (0),
		ADD    (1),
		MODIY  (2),
	...
	private static BookManager_backup instance;
	...
	private BookManager_backup() {
	   setupUserData();
	   setupBookStore();
	   lastIndex = 4;
	}
	
	public static BookManager_backup getInstance() {
		if (instance == null) //생성되지 않았으면 생성.
			instance = new BookManager_backup();
...
//    .setPrice(35000).build();
	...
	public BookManager_backup setup(String userId, String password) {
		login(userId, password);
		return this;
	}
	public BookManager_backup setup(Scanner s, String userId, String password) {
		scn = s;
		login(userId, password);
		return this;
	}
